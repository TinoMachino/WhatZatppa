...
function buildTagsArray(
  ...
  // Add post type tag if present
  if (draft.postType?.tag) {
    tags.push(draft.postType.tag)
  }

  // Add matter/policy type tag if present (legacy support or if still populated)
  if (
    draft.matterPolicyType &&
    MATTER_POLICY_TAGS[
    draft.matterPolicyType as keyof typeof MATTER_POLICY_TAGS
    ]
  ) {
    tags.push(
      MATTER_POLICY_TAGS[
...
export async function post(
  ...
  for (let i = 0; i < thread.posts.length; i++) {
    ...
    writes.push({
      ...
      collection: opts.collection || 'app.bsky.feed.post',
      rkey: rkey,
      value: record,
    })
...
> {
  if (embedDraft.media?.type === 'images') {
    ...
    const isGif = imagesDraft.some(
      i => (i.transformed?.mime ?? i.source.mime) === 'image/gif',
    )
    onStateChange?.(isGif ? t`Uploading GIF...` : t`Uploading images...`)
  ...
  if (
    ...
    return {
      ...
      aspectRatio,
      presentation:
        videoDraft.video.mimeType === 'image/gif' ? 'gif' : undefined,
    }
  }
