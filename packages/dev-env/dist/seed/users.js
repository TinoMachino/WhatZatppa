...
exports.default = async (sc) => {
    ...
    await sc.createProfile(sc.dids.alice, users.alice.displayName, users.alice.description, users.alice.selfLabels);
    await sc.createProfile(sc.dids.bob, users.bob.displayName, users.bob.description, users.bob.selfLabels);
    await sc.agent.api.chat.bsky.actor.declaration.create({ repo: sc.dids.dan }, { allowIncoming: 'none' }, sc.getHeaders(sc.dids.dan));
    return sc;
};
const users = {
    alice: {
