...
exports.default = async (sc) => {
    ...
    await sc.verify(sc.dids.verifier1, sc.dids.bob, sc.accounts[sc.dids.bob].handle, sc.profiles[sc.dids.bob].displayName);
    await sc.verify(sc.dids.verifier2, sc.dids.bob, sc.accounts[sc.dids.bob].handle, sc.profiles[sc.dids.bob].displayName);
    // carol: has non-broken and broken verifications by verifiers.
    await sc.verify(sc.dids.verifier1, sc.dids.carol, sc.accounts[sc.dids.carol].handle, sc.profiles[sc.dids.carol].displayName);
    await sc.verify(sc.dids.verifier2, sc.dids.carol, 'carol.old.handle', // Broken: this was the handle during verification and it changed later.
    sc.profiles[sc.dids.carol].displayName);
    // dan: has verifications by verifiers and non verifiers.
    await sc.verify(sc.dids.verifier1, sc.dids.dan, sc.accounts[sc.dids.dan].handle, sc.profiles[sc.dids.dan].displayName);
    await sc.verify(sc.dids.nonverifier, sc.dids.dan, sc.accounts[sc.dids.dan].handle, sc.profiles[sc.dids.dan].displayName);
    // eve: has no verifications at all.
    // NOOP
    // frank: has only broken verifications by verifiers.
    await sc.verify(sc.dids.verifier2, sc.dids.frank, sc.accounts[sc.dids.frank].handle, 'frank-old-name');
    // gus: has only verifications by non verifiers.
    await sc.verify(sc.dids.nonverifier, sc.dids.gus, sc.accounts[sc.dids.gus].handle, sc.profiles[sc.dids.gus].displayName);
    // impersonator: has verification by verifier but should get the impersonator label during tests.
    await sc.verify(sc.dids.verifier1, sc.dids.impersonator, sc.accounts[sc.dids.impersonator].handle, sc.profiles[sc.dids.impersonator].displayName);
