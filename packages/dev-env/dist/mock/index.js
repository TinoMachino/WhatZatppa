...
async function generateMockSetup(env) {
    const date = dateGen();
    ...
    await reporter.com.atproto.moderation.createReport({
        reasonType: picka([
            api_1.COM_ATPROTO_MODERATION.DefsReasonSpam,
            api_1.COM_ATPROTO_MODERATION.DefsReasonOther,
        ]),
    ...
    const posts = [];
    for (let i = 0; i < data_1.postTexts.length; i++) {
        const author = picka(userAgents);
        const post = await author.app.bsky.feed.post.create({ repo: author.did }, {
            text: data_1.postTexts[i],
            createdAt: date.next().value,
        });
        posts.push(post);
        if (rand(10) === 0) {
            const reposter = picka(userAgents);
            await reposter.app.bsky.feed.repost.create({ repo: reposter.did }, {
                subject: picka(posts),
                createdAt: date.next().value,
            });
        }
    ...
    const file = Buffer.from(labeled_img_b64_1.default, 'base64');
    ...
    for (let i = 0; i < 100; i++) {
        ...
        const urip = new syntax_1.AtUri(targetUri);
        const target = await alice.app.bsky.feed.post.get({
            repo: urip.host,
            rkey: urip.rkey,
        });
        const author = picka(userAgents);
        posts.push(await author.app.bsky.feed.post.create({ repo: author.did }, {
            text: picka(data_1.replyTexts),
            reply: {
    ...
    for (const post of posts) {
        for (const user of userAgents) {
            if (rand(3) === 0) {
                await user.app.bsky.feed.like.create({ repo: user.did }, {
                    subject: post,
                    createdAt: date.next().value,
                });
            }
    ...
    const avatarImg = Buffer.from(blur_hash_avatar_b64_1.default, 'base64');
                        ...
                        locales: [
                            {
                                lang: 'en',
                                name: 'Spam',
                                description: 'Low quality posts that dont add to the conversation.',
                            },
                        ],
                    },
                    {
...
const createLabel = async (db, opts) => {
    await db.db
        ...
        neg: false,
        src: opts.src ?? index_1.EXAMPLE_LABELER,
    })
        .execute();
};
const setVerifier = async (db, did) => {
    await db.db
        .updateTable('actor')
