...
class DiskBlobStore {
    ...
    async makePermanent(key, cid) {
        ...
        try {
            ...
            logger_1.blobStoreLogger.error({ err, tmpPath }, 'could not delete file from temp storage');
        }
    }
    ...
    async deleteMany(cids) {
        const errors = [];
        for (const chunk of (0, common_1.chunkArray)(cids, 500)) {
            await Promise.all(chunk.map((cid) => this.delete(cid).catch((err) => {
                logger_1.blobStoreLogger.error({ err, cid: cid.toString() }, 'error deleting blob');
                errors.push(err);
            })));
        }
        if (errors.length)
            throw (0, common_1.aggregateErrors)(errors);
    }
    async deleteAll() {
