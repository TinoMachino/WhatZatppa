...
class AccountManager {
    ...
    async createAccount({ did, handle, email, password, repoCid, repoRev, inviteCode, deactivated, refreshJwt, }) {
        if (password && password.length > scrypt.NEW_PASSWORD_MAX_LENGTH) {
            throw new xrpc_server_1.InvalidRequestError('Password too long');
        }
        const passwordScrypt = password
            ? await scrypt.genSaltAndHash(password)
            : undefined;
        const now = (0, syntax_1.currentDatetimeString)();
    ...
    async createAccountAndSession(opts) {
        const { accessJwt, refreshJwt } = await auth.createTokens({
            ...
            scope: auth_scope_1.AuthScope.Access,
        });
    ...
    async login({ identifier, password, }) {
        ...
        try {
            ...
            const user = identifierNormalized.includes('@')
                ...
                : (0, lex_1.isAtIdentifierString)(identifierNormalized)
