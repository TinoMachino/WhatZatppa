...
const createAccessToken = (opts) => {
    const { did, jwtKey, serviceDid, scope = auth_scope_1.AuthScope.Access, expiresIn = '120mins', } = opts;
...
const createRefreshToken = (opts) => {
    ...
    const signer = new jose.SignJWT({ scope: auth_scope_1.AuthScope.Refresh })
        .setProtectedHeader({
        typ: 'refresh+jwt',
        alg: 'HS256', // only symmetric keys supported
    })
...
const decodeRefreshToken = (jwt) => {
    ...
    node_assert_1.default.ok(token.scope === auth_scope_1.AuthScope.Refresh, 'not a refresh token');
    return token;
};
...
const formatScope = (appPassword, isSoftDeleted) => {
    if (isSoftDeleted)
        return auth_scope_1.AuthScope.Takendown;
    if (!appPassword)
        return auth_scope_1.AuthScope.Access;
    return appPassword.privileged
        ? auth_scope_1.AuthScope.AppPassPrivileged
        : auth_scope_1.AuthScope.AppPass;
};
