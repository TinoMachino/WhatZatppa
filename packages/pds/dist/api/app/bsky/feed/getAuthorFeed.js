    ...
    server.add(index_js_1.app.bsky.feed.getAuthorFeed, {
        auth: ctx.authVerifier.authorization({
            authorize: (permissions, { req }) => {
                const lxm = index_js_1.app.bsky.feed.getAuthorFeed.$lxm;
                const aud = (0, pipethrough_1.computeProxyTo)(ctx, req, lxm);
                permissions.assertRpc({ aud, lxm });
            },
        }),
        handler: async (reqCtx) => {
            return (0, read_after_write_1.pipethroughReadAfterWrite)(ctx, reqCtx, index_js_1.app.bsky.feed.getAuthorFeed, getAuthorMunge);
        },
    });
}
...
const isUsersFeed = (feed, requester) => {
    ...
    if (!first.reason && first.post.author.did === requester)
        return true;
    if (first.reason &&
        index_js_1.app.bsky.feed.defs.reasonRepost.$isTypeOf(first.reason) &&
