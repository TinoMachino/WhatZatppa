...
function default_1(server, ctx) {
    ...
    server.add(index_js_1.com.atproto.server.requestEmailUpdate, {
        rateLimit: [
            {
        ...
        handler: async ({ auth, req }) => {
            ...
            if (entrywayClient) {
                const { headers } = await ctx.entrywayAuthHeaders(req, auth.credentials.did, index_js_1.com.atproto.server.requestEmailUpdate.$lxm);
                return entrywayClient.xrpc(index_js_1.com.atproto.server.requestEmailUpdate, {
                    headers,
                });
            }
            if (!account.email) {
