...
function default_1(server, ctx) {
    server.add(index_js_1.com.atproto.admin.updateAccountHandle, {
        ...
        handler: async ({ input }) => {
            ...
            if (account) {
                ...
                if (ctx.cfg.entryway) {
                    ...
                    if (doc?.handle !== handle) {
                        throw new xrpc_server_1.InvalidRequestError('Handle does not match DID doc');
                    }
                }
                else {
            ...
            try {
                ...
                logger_1.httpLogger.error({ err, did, handle }, 'failed to sequence handle update');
            }
        },
    });
}
