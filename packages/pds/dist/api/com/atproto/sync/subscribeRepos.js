...
function default_1(server, ctx) {
    server.add(index_js_1.com.atproto.sync.subscribeRepos, async function* ({ params, signal, }) {
        const { cursor } = params;
        ...
        if (cursor !== undefined) {
            ...
            if (cursor > (curr ?? 0)) {
                ...
                yield index_js_1.com.atproto.sync.subscribeRepos.info.$build({
                    name: 'OutdatedCursor',
        ...
        for await (const evt of outbox.events(outboxCursor, signal)) {
            if (evt.type === 'commit') {
                yield index_js_1.com.atproto.sync.subscribeRepos.commit.$build({
                    seq: evt.seq,
                    time: evt.time,
                    ...evt.evt,
                });
                ...
                yield index_js_1.com.atproto.sync.subscribeRepos.sync.$build({
                    seq: evt.seq,
                    time: evt.time,
                    ...evt.evt,
                });
                ...
                yield index_js_1.com.atproto.sync.subscribeRepos.identity.$build({
                    seq: evt.seq,
                    time: evt.time,
                    ...evt.evt,
                });
                ...
                yield index_js_1.com.atproto.sync.subscribeRepos.account.$build({
                    seq: evt.seq,
                    time: evt.time,
                    ...evt.evt,
                });
