...
const publishIdentity = async (ctx, args) => {
    const dids = args;
    (0, node_assert_1.default)(dids.every(lex_1.isDidString), 'All arguments must be DIDs');
...
const publishIdentityFromFile = async (ctx, args) => {
    ...
    const dids = file
        ...
        .split('\n')
        .map((did) => did.trim());
    (0, node_assert_1.default)(dids.every(lex_1.isDidString), 'File contains invalid DIDs');
