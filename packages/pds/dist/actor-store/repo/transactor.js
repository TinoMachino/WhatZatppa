...
class RepoTransactor extends reader_1.RepoReader {
    ...
    async maybeLoadRepo() {
        const res = await this.db.db
            ...
            .limit(1)
            .executeTakeFirst();
        return res ? repo_1.Repo.load(this.storage, (0, lex_data_1.parseCid)(res.cid)) : null;
    }
    ...
    async formatCommit(writes, swapCommit) {
        ...
        for (const write of writes) {
            ...
            const currRecord = record ? (0, lex_data_1.parseCid)(record.cid) : null;
            const op = {
                action,
    ...
    async getDuplicateRecordCids(cids, touchedUris) {
        ...
        const res = await this.db.db
            ...
            .select('cid')
            .execute();
        return res.map((row) => (0, lex_data_1.parseCid)(row.cid));
    }
}
