...
class SqlRepoReader extends repo_1.ReadableBlockstore {
    ...
    async getRootDetailed() {
        ...
        return {
            cid: (0, lex_data_1.parseCid)(res.cid),
            rev: res.rev,
        };
    }
    async getBytes(cid) {
    ...
    async getBlocks(cids) {
        ...
        for (const batch of (0, common_1.chunkArray)(missingStr, 500)) {
            const res = await this.db.db
                .selectFrom('repo_block')
            ...
            for (const row of res) {
                const cid = (0, lex_data_1.parseCid)(row.cid);
    ...
    async *iterateCarBlocks(since) {
        ...
        do {
            ...
            if (lastRow && lastRow.repoRev) {
                cursor = {
                    cid: (0, lex_data_1.parseCid)(lastRow.cid),
                    rev: lastRow.repoRev,
                };
            }
            else {
