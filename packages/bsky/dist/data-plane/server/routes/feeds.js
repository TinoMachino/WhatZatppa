...
exports.default = (db) => ({
    async getParaAuthorFeed(req) {
        ...
        const keyset = new pagination_1.TimeCidKeyset(ref('para_post.sortAt'), ref('para_post.cid'));
        ...
        const [parents, replies] = await Promise.all([
            post.replyRoot || post.replyParent
                ? db.db
                    .selectFrom('para_post')
                    .selectAll('para_post')
                    .where('uri', 'in', [post.replyRoot, post.replyParent].filter((uri) => !!uri))
                    .orderBy('sortAt', 'asc')
                    .limit(req.above || 80)
                    .execute()
                : [],
...
const paraFeedItemFromRow = (row) => {
    return new bsky_pb_1.ParaAuthorFeedItem({
        uri: row.uri,
        cid: row.cid,
        author: row.creator,
        text: row.text,
