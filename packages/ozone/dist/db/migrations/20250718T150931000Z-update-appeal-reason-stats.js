...
async function up(db) {
    ...
    await db.schema
        ...
        (eb) => (0, kysely_1.sql) `COUNT(*) FILTER(
              ...
              AND ${eb.ref('meta')} ->> 'reportType' NOT IN (${defs_1.REASONAPPEAL}, ${util_1.OZONE_APPEAL_REASON_TYPE})
            )`.as('reportCount'),
        (eb) => (0, kysely_1.sql) `COUNT(*) FILTER(
              WHERE ${eb.ref('action')} = 'tools.ozone.moderation.defs#modEventReport'
              AND ${eb.ref('meta')} ->> 'reportType' IN (${defs_1.REASONAPPEAL}, ${util_1.OZONE_APPEAL_REASON_TYPE})
            )`.as('appealCount'),
    ])
        .groupBy('subjectDid'))
        .execute();
    ...
    await db.schema
        ...
        (eb) => (0, kysely_1.sql) `COUNT(*) FILTER (WHERE ${eb.ref('action')} = 'tools.ozone.moderation.defs#modEventReport' AND ${eb.ref('meta')} ->> 'reportType' NOT IN (${defs_1.REASONAPPEAL}, ${util_1.OZONE_APPEAL_REASON_TYPE}))`.as('reportCount'),
        (eb) => (0, kysely_1.sql) `COUNT(*) FILTER (WHERE ${eb.ref('action')} = 'tools.ozone.moderation.defs#modEventReport' AND ${eb.ref('meta')} ->> 'reportType' IN (${defs_1.REASONAPPEAL}, ${util_1.OZONE_APPEAL_REASON_TYPE}))`.as('appealCount'),
    ])
    ...
    await db.schema
        ...
        .select([
        'subjectDid',
        (eb) => (0, kysely_1.sql) `SUM(${eb.ref('reportCount')})::bigint`.as('totalReports'),
...
//# sourceMappingURL=20250718T150931000Z-update-appeal-reason-stats.js.map