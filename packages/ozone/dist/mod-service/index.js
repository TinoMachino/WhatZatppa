...
class ModerationService {
    ...
    async logEvent(info) {
        ...
        if ((0, defs_1.isAgeAssurancePurgeEvent)(event)) {
    ...
    async purgeAgeAssuranceEvents(subjectDid) {
        this.db.assertTransaction();
        await this.db.db
    ...
    async getAccountTimeline(did) {
        ...
        const result = await this.db.db
            .selectFrom(this.db.db
            .selectFrom('moderation_event')
            .where('subjectDid', '=', did)
            .select([
            (0, util_3.dateFromDbDatetime)(ref('createdAt')).as('day'),
            'subjectUri',
            'action',
...
const parseTags = (tags) => tags
    ...
    .filter((subTags) => subTags.length > 0);
const TAKEDOWNS = ['pds_takedown', 'appview_takedown'];
exports.TAKEDOWN_LABEL = '!takedown';
exports.SUSPEND_LABEL = '!suspend';
