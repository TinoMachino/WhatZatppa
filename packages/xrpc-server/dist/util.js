...
function createLexiconInputVerifier(nsid, def, options, lexicons) {
    ...
    (0, node_assert_1.default)(!options.inputProcessingOptions, 'inputProcessingOptions is not supported for lexicon based methods. Use server.add() instead.');
    ...
    const checkEncoding = allowedEncodings.includes(ENCODING_ANY)
        ...
        : (encoding) => allowedEncodings.includes(encoding);
        ...
        if (input.schema) {
            try {
                const lexBody = req.body
                    ? (0, lex_json_1.jsonToLex)(req.body, inputProcessingOptions)
                    : req.body;
...
function parseReqEncoding(req) {
    ...
    if (encoding)
        return encoding;
    throw new errors_1.InvalidRequestError(`Request encoding (Content-Type) required but not provided`);
}
function normalizeMime(v) {
    if (!v)
        return null;
    const fullType = (0, mime_types_1.contentType)(v);
    if (!fullType)
        return null;
...
const ENCODING_ANY = '*/*';
function parseDefEncoding({ encoding }) {
...
function createBodyParser(inputEncoding, options) {
    ...
    const jsonParser = (0, express_1.json)({ limit: jsonLimit });
    const textParser = (0, express_1.text)({ limit: textLimit });
    // Transform json and text parser middlewares into a single function
    return (req, res) => {
        return new Promise((resolve, reject) => {
            jsonParser(req, res, (err) => {
                if (err)
                    return reject(errors_1.XRPCError.fromError(err));
...
function decodeBodyStream(req, maxSize) {
    ...
    if (Number.isNaN(contentLengthParsed)) {
        throw new errors_1.XRPCError(xrpc_1.ResponseType.InvalidRequest, 'invalid content-length');
    }
    if (maxSize !== undefined &&
        contentLengthParsed !== undefined &&
        contentLengthParsed > maxSize) {
        throw new errors_1.XRPCError(xrpc_1.ResponseType.PayloadTooLarge, 'request entity too large');
    }
    let transforms;
    try {
        ...
        throw new errors_1.XRPCError(xrpc_1.ResponseType.UnsupportedMediaType, 'unsupported content-encoding', undefined, { cause });
    }
    if (maxSize !== undefined) {
        const maxSizeChecker = new common_1.MaxSizeChecker(maxSize, () => new errors_1.XRPCError(xrpc_1.ResponseType.PayloadTooLarge, 'request entity too large'));
...
const parseUrlNsid = (url) => {
    const nsid = (0, exports.extractUrlNsid)(url);
    if (nsid)
        return nsid;
    throw new errors_1.InvalidRequestError('invalid xrpc path');
};
