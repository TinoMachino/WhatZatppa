...
const BaseTab = createNativeStackNavigatorWithAuth<BaseTabNavigatorParams>()
  ...
  return (
    <>
      ...
      <Stack.Screen
        ...
        options={{title: title(msg`Cabildeo`), requireAuth: true}}
      />
      <Stack.Screen
        name="CabildeoDetail"
      ...
      <Stack.Screen
        ...
        options={{title: title(msg`Crear Cabildeo`), requireAuth: true}}
      />
      <Stack.Screen
        name="CreatePosition"
        getComponent={() => CreatePositionScreen}
        options={{title: title(msg`Tomar Postura`), requireAuth: true}}
      />
    </>
  )
}
...
function TabsNavigator({
  ...
  return (
    <Tab.Navigator
      ...
      <Tab.Screen name="BaseTab" getComponent={() => BaseTabNavigator} />
      <Tab.Screen
...
function BaseTabNavigator() {
  const t = useTheme()
  return (
    <BaseTab.Navigator screenOptions={screenOptions(t)} initialRouteName="Base">
      <BaseTab.Screen name="Base" getComponent={() => BaseScreen} />
      <BaseTab.Screen
        name="MyBase"
      ...
      />
      {commonScreens(BaseTab as typeof Flat)}
    </BaseTab.Navigator>
  )
}
...
const LINKING = {
  ...
  getStateFromPath(path: string) {
    ...
    if (IS_NATIVE) {
      ...
      if (name === 'Base') {
        return buildStateObject('BaseTab', 'Base', params)
      }
...
function resetToTab(
  tabName: 'HomeTab' | 'SearchTab' | 'BaseTab' | 'NotificationsTab',
) {
  if (navigationRef.isReady()) {
    navigate(tabName)
